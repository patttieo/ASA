...
[![npm module downloads per month](//img.shields.io/npm/dm/ignore.svg)](https://www.npmjs.org/package/ignore)

# ignore

`ignore` is a manager and filter which implemented in pure JavaScript according to the .gitignore [spec](//git-scm.com/docs/gitignore).
...
3. Exactly according to [gitignore man page](//git-scm.com/docs/gitignore), fixes some known matching issues of fstream-ignore, such as:
...
Usually, you could use [`glob`](//npmjs.org/package/glob) with `option.mark = true` to fetch the structure of the current directory:
